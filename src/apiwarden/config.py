...
# Keys accepted from settings.APIWARDEN, apiwarden.toml, or CLI flags.
_KEYS = (
    "root",
    "title",
    "servers",
...
def from_dict(values: dict[str, Any]) -> Config:
    ...
    if unknown:
        raise ValueError(f"unknown apiwarden settings: {', '.join(sorted(unknown))}")
...
def from_toml(path: Path) -> Config:
    ...
    section = data.get("apiwarden", data)
    return from_dict({k: v for k, v in section.items() if k in _KEYS})


def find_toml(start: Path) -> Path | None:
    """Look for apiwarden.toml beside the spec root, then in its parents."""
    for directory in (start, *start.parents):
        candidate = directory / "apiwarden.toml"
        if candidate.exists():
            return candidate
...
def env_token() -> str | None:
    return os.environ.get("APIWARDEN_TOKEN") or None
