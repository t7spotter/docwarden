...
```
pip install apidocs-live
apidocs serve ./api-docs
```
...
{
  "mcpServers": {
    "apidocs": { "type": "http", "url": "https://your-host/api-docs/mcp" }
  }
}
```
...
```
apidocs mcp ./api-docs
```

## Standalone

```
apidocs serve ./api-docs              # http://127.0.0.1:8080, reloads as you edit
apidocs check ./api-docs              # lint: operationIds, summaries, unresolved $refs
apidocs build ./api-docs -o dist/     # self-contained static copy, for CI publishing
apidocs changes ./api-docs --since v1.4.0
apidocs snapshot ./api-docs -o baseline.json
```
...
`apidocs snapshot`. `--fail-on-breaking` exits non-zero, so CI can gate on it.
...
APIDOCS_LIVE = {
    ...
    "watch": DEBUG,
    "token": os.environ.get("APIDOCS_TOKEN"), # omit for a public portal
}
...
Settings are the same for both, via `APIDOCS_LIVE`, an `apidocs.toml` beside the
specs, or CLI flags.
...
`apidocs serve` when you try things out.
